...
Der \emph{idle}-Zustand ist dabei nicht notwendigerweise "`im Stillstand"': Da in den frühen Entwicklungsphasen --- nicht zuletzt wegen des in Abschnitt~\ref{subsec:mpu6050_lessonslearned} 
...
\label{subsec:verarbeitung_lessonslearned}

Wie in Abbildung~\ref{fig:codewarrior} bereits zu erkennen, war es notwendig, den Interrupt Handler der UART gesondert zu behandeln. Dieser überprüft, ob Sende- und Empfangsregister
leer respektive voll sind und leitet dann entsprechende Aktionen auf dem Sende- und Empfangspuffer ein. Wann immer jedoch für den Compiler eine Optimierungsstufe größer als 
\texttt{-O0} ("`keine Optimierung"') verwendet wurde, wurde das Timing der Operation so verändert, dass nach ausgelöstem \texttt{RDRF}-Interrupt (Receive Data Register Full, d.h. ein Datenempfang
hat stattgefunden) das Empfangsregister \emph{leer} war. Ein Test mit einem längerem Datenstrom zeigte, dass nur das zweite Byte ders Stroms korrekt empfangen wurde.
Dieses Problem war erst dadurch zu beheben, dass der (eigentlich zeitkritische) Interrupt-Handler von der Optimierung ausgenommen wurde.

Dies wurde durch das Anbringen des gcc-spezifischen Attributs \texttt{optimize} erreicht, wie es in Listing~\ref{lst:unoptimized-irq} verdeutlicht ist.

\begin{lstlisting}[style=lolc,caption={Deaktivieren der Optimierung für eine Interruptroutine},label=lst:unoptimized-irq]
...
\subsubsection{Sensorrohdaten}{Sensorrohdaten (Modus 0)}
...
\subsubsection{Roll-Pitch-Yaw}{Roll-Pitch-Yaw (Modus 42)}
...
\subsubsection{Orientierungs-Quaternion}{Orientierungs-Quaternion (Modus 43)}
...
\subsubsection{Orientierungs-Quaternion und Roll-Pitch-Yaw}{Orientierungs-Quaternion und Roll-Pitch-Yaw (Modus 44)}
...
Nutzdaten nicht vorkommen. Tritt ein solcher Wert auf, wird das \gls{esc} (\texttt{0x1B}) eingefügt
...
nicht als Klasse implementiert sein. Dies bringt extreme Performanceeinbußen mit sich, was der internen Handhabung von Klassen in MATLAB geschuldet ist.
