...
\begin{figure}[htbp]
	...
	\caption[kalman-clib in Aktion]{kalman-clib in Aktion: Ausgabe des Codes in Listing~\ref{lst:kalman-clib} zur Schätzung der Gravitationskonstante nach 15 Iterationen}
...
befüllt werden und daher nicht als Kandidaten für den ROM gelten können (d.h. nicht \gls{romable} sind). Die Berechnung der Sinus- und Kosinusfunktionen erfolgt dann bei jedem Aufruf 
wahlweise\footnote{Dies wird durch das Preprozessorflag \texttt{FIXMATH\_FAST\_SIN} gesteuert.} über eine drei- oder fünfstufige Taylorapproximation.

Die verwendeten Settings zum Setzen der maximalen Matrixgröße, Deaktivieren der Funktionen für Filter mit Steuereingängen\footnote{Dies ist nicht nötig, reduziert aber auch ohne Optimierungen die Codegröße.}
und Deaktivierung der Look-up-Tabellen finden sich in Listing~\ref{lst:libfixkalman-defines} (vgl. Abbildung~\ref{fig:visualgd_makefile}).
...
\section{Implementierung des Fusionstreibers}
\label{sec:impl_fusion}

Der Fusionstreiber selbst besteht aus mehreren Teilkomponenten und ist als adaptives System implementiert, welches eine optimale Fusionsstrategie in Abhängigkeit von den vorliegenden
Sensordaten wählt.

Hierzu stehen dem Benutzer drei Funktionen zum Setzen der Sensordaten zur Verfügung, mit welchen die jeweils vorliegenden Sensordaten registriert werden können (Listing~\ref{lst:fusion_register}).

\begin{lstlisting}[style=lolc,caption={Fusion: Registrierung der Sensordaten},label=lst:fusion_register]
void fusion_set_accelerometer(
	register const fix16_t *const ax, 
	register const fix16_t *const ay, 
	register const fix16_t *const az);
void fusion_set_gyroscope(
	register const fix16_t *const gx, 
	register const fix16_t *const gy, 
	register const fix16_t *const gz);
void fusion_set_magnetometer(
	register const fix16_t *const mx, 
	register const fix16_t *const my, 
	register const fix16_t *const mz);
\end{lstlisting}

Ein Aufruf jeder dieser Funktionen registriert einen neuen Sensorwert für den Korrekturschritt des Filters. Hierbei obliegt es dem Benutzer, die Daten zuvor geeignet in 
das \gls{q16}-Format zu konvertieren.

Mittels der in Listing~\ref{lst:fusion_predict_interface} dargestellten Funktionen wird dem Benutzer Zugriff auf die Kernfunktionalität des Fusiontreibers gegeben, wobei in zyklischer
Abfolge die Funktionen \texttt{fusion\_predict()} (schnell, aber ungenau) und \texttt{fusion\_correct()} (langsam, aber genau) aufgerufen werden müssen.

\begin{lstlisting}[style=lolc,caption={Fusion: Schnittstelle zur Prädiktion},label=lst:fusion_predict_interface]
void fusion_initialize();
void fusion_predict(register const fix16_t deltaT);
void fusion_update(register const fix16_t deltaT);
\end{lstlisting}

Die Implementierung von \texttt{fusion\_correct()} kann Listing~\ref{lst:fusion_adaptive} entnommen werden. Es ist zu erkennen, dass in Abhängigkeit des Vorhandenseins von
Sensorwerten entweder keine Korrektur, eine Korrektur nur anhand der Gyrosensor-Daten\footnote{Dies ist ein zusätzlicher Fall, da der Gyrosensor Daten in sehr viel höherer Frequenz liefern kann.}
oder eine Mischung aus Auf- und Vorwärtsvektorkorrektur vorgenommen wird. Die Trennung in \emph{attitude}- und \emph{orientation}-Filter entspricht dabei der in 
Abschnitt~\ref{subsec:indir_filt_est_orient} dargestellten Trennung zwischen Filter zur Ermittlung der Neigung aus dem Aufwärtsvektor und dem Erweiterungsfilter mit der Vorwärtsrichtung 
zur Ermittlung der vollständigen Orientierung.

\begin{lstlisting}[style=lolc,caption={Fusion: Adaptiver Updatemechanismus},label=lst:fusion_adaptive]
void fusion_update(register const fix16_t deltaT)
{
...
Die Implementierung von \texttt{fusion\_update\_orientation()} ist dabei in Listing~\ref{lst:magnetometer_correct} zu finden, die zugehörige Hilfsfunktion \texttt{magnetometer\_project()}
ergänzend in Listing~\ref{lst:magnetometer_project}.

Zu erkennen ist, dass die Fusionsfunktionen eine Differenzzeit $\Delta t$ (\texttt{deltaT}) benötigen. Die hierfür notwendige Zeitmessung wurde über den \gls{systick}-Timer realisiert.


\section{Test der Fusion}
\label{sec:test_fusion}

Um die korrekte Funktionsweise der Fusion zu überprüfen, wurde in C\# unter Zuhilfenahme von OpenGL eine Anwendung geschrieben, welche das in Abschnitt~\ref{sec:serial_transmission} beschriebene
Übertragungsprotokoll implementiert (vgl. Decoder in Listing~\ref{lst:csharp-p2ppdecoder}) und die fusionierten bzw. unfusionierten Ergebnisse darstellt. Ein Foto eines Testlaufes ist
in Abbildung~\ref{fig:fused-csharp} zu sehen\footnote{Ein Video des Testlaufes kann unter \url{http://youtu.be/tmC5kjQn-uA} gefunden werden.}.
...
Auf die Implementierungsdetails der Testanwendung (OpenGL-Koordinaten\-trans\-formation, etc.) soll jedoch nicht weiter eingegangen werden, da es sich bei dieser nur um einen Nebenschauplatz der Entwicklung handelte.
