...
(etwa eine Neigung \glslink{symb:theta}{$\theta$}) exakt anzugeben, wobei für die verbleibenden Winkel lediglich eine Bedingung der Art $\left|\phi + \psi \right| \leq 180^\circ$ angegeben werden kann.
...
ein Nickwinkel (Pitch, \gls{Elevation}) von $\theta = \pm 90^\circ$ zu einer Singularität auf den Achsen \glslink{symb:phi}{$\phi$} (Yaw, \gls{Azimut}) und \glslink{symb:psi}{$\psi$} (Roll) führte.
...
Hierbei werden zwei separate \gls{Kalman-Filter} eingesetzt, um die Aspekte der Neigung und der horizontalen Auslenkung dezentral zu schätzen (vgl. \citealp{TiborKalman}), was zur
Reduktion der Komplexität beiträgt.

\cite{Tsang} und \cite{Filieri} beschreiben weiterhin ein Fehlermodell zur Schätzung der Beschleunigungs- und Integrationsunsicherheiten --- diese machen einen nicht unbeträchtlichen Teil der Unsicherheit aus --- 
welches unterstützend auch zur direkten Schätzung der Sensordrift angewendet werden kann. Dieser Aspekt wurde in MATLAB erfolgreich untersucht, jedoch nicht in der Firmware implementiert.
