...
\glslink{symb:R}{$\underline(R)$} nutzt diesen Zusammenhang zur Darstellung alle Winkelzusammenhänge zweier Systeme $\underline{S}_{\text{body}} = \left\{\vec{x}, \vec{y}, \vec{z} \right\}$, 
...
Wird das Koordinatensystem \glslink{symb:xxxyxz}{$\underline{S}$} in Matrixform definiert, so dass
...
Orientierungserkennung aus drei\footnote{Hieraus ergibt sich der Name des Algorithmus.} Vektorbeobachtungen \glslink{symb:xyz}{$\vec{x}, \vec{y}, \vec{z}$}. 
...
In diesem Fall ergibt sich die \gls{dcm} $\underline{R}$ mittels Gleichung~\ref{eq:dcm_short} direkt als 
...
\label{subsec:herleitung_vektorbeobachtungen}
...
Die Renormalisierung von $\vec{y}$ ist dabei mathematisch dann nicht notwendig, wenn $\vec{m}$ und $\vec{z}$ bereits normalisiert vorliegen. Sie sollte 
...
\subsection{Herleitung der Winkel aus der DCM}
\label{subsec:herleitung_winkel_dcm}

Werden die \textsc{Euler}'schen Winkel aus der \gls{dcm} $\underline{R}$ benötigt, können diese nach \cite{premeul} über die folgenden Zusammenhänge\footnote{Unter der Annahme der Tait-Bryan $X\text{--}Y\text{--}Z$-Folge} gewonnen werden.

\begin{align}
\theta_{\text{pitch}} &= -\sin^{-1} \underline{R}_{0,2} = z_x \label{eq:pitch_from_dcm} \\
\phi_{\text{roll}}   &= \tan^{-1} \frac{\underline{R}_{1,2}}{\underline{R}_{2,2}} = \tan^{-1} \frac{z_y}{z_z} \\
\psi_{\text{yaw}}   &= \tan^{-1} \frac{\underline{R}_{0,1}}{\underline{R}_{0,0}} = \tan^{-1} \frac{y_x}{x_x}
\end{align}