...
\begin{table}[htbp]
	\centering
		\begin{tabular}{ll}
		\toprule
		\multicolumn{2}{l}{Eigenschaften des Freescale MMA8451Q} \\
		\midrule
		Messbereich &  $\pm2g$, $\pm4g$ oder $\pm8g$ \\
		Samplingrate &  \unit{1,56}{\hertz} bis \unit{800}{\hertz}\\
		Auflösung & 8-Bit oder 14-Bit \\
		Rauschen & \unit{99}{\micro g/\sqrt{\hertz}} \\
		Versorgungsspannung & \unit{1,95}{\volt} bis \unit{3,6}{\volt} \\
		Schnittstellenspannung & \unit{1,6}{\volt} bis \unit{3,6}{\volt} \\
		Stromverbrauch & \unit{6}{\micro\ampere} bis \unit{165}{\micro\ampere} \\
		Betriebstemperatur & \unit{-40}{\celsius} bis \unit{+85}{\celsius} \\
		\bottomrule
		\end{tabular}
	\caption{Eigenschaften des Freescale MMA8451Q}
	\label{tab:MMA8451Eigenschaften}
\end{table}
...
Der Sensor wurde während des Testlaufes auf eine Empfindlichkeit von $\pm 2g$ (d.h. $\pm 2\times\unit{9,81}{\meter/\second^2}$ peak-to-peak) bei einer Samplingrate von \unit{100}{\hertz} eingestellt.
Die Triggerung der Verbarbeitung wurde über eine der Interruptleitungen vorgenommen, welche auf open drain, active low im \emph{data ready}-Modus geschaltet wurde.
...
Da für jede Achse nun zwei Varianzmessungen vorliegen, können diese mittels der \glslink{pooled variance}{"`pooled"'-Varianzmethode} als erwartungstreu gewichtete Mittelung zusammengeführt werden, so dass
...
\begin{table}[htbp]
	\centering
		\begin{tabular}{ll}
		\toprule
		\multicolumn{2}{l}{Eigenschaften der Invensense MPU6050} \\
		\midrule
		Versorgungsspannung (VDD) & \unit{2,375}{\volt} bis \unit{3,46}{\volt} \\
		Schnittstellenspannung & \unit{1,71}{\volt} bis VDD \\
		Auflösung & 16-Bit \\
		Betriebstemperatur & \unit{-40}{\celsius} bis \unit{+85}{\celsius} \\
		\midrule
		\multicolumn{2}{l}{Eigenschaften des Gyrosensors} \\
		\midrule
		Messbereich & \unit{±250, ±500, ±1000, und ±2000}{\degree/\second} \\
		Samplingrate & \unit{4}{\hertz} bis \unit{8}{\kilo\hertz} \\
		Rauschen & \unit{0,005}{\degree/\second/\sqrt{\hertz}} \\
		Stromverbrauch & \unit{3,6}{\milli\ampere} (Standby \unit{5}{\micro\ampere}) \\
		\midrule
		\multicolumn{2}{l}{Eigenschaften des Accelerometers} \\
		\midrule
		Messbereich & ±2g, ±4g, ±8g und ±16g \\
		Samplingrate & \unit{4}{\hertz} bis \unit{1}{\kilo\hertz} \\
		Rauschen & \unit{400}{\micro\gram/\sqrt{\hertz}} \\
		Stromverbrauch & \unit{10}{\micro\ampere} bis \unit{500}{\micro\ampere} \\		
		\bottomrule
		\end{tabular}
	\caption{Eigenschaften der Invensense MPU6050}
	\label{tab:MPU6050Eigenschaften}
\end{table}
...
Der Accelerometer wurde auf eine Empfindlichkeit von $\pm 4g$ (d.h. $\pm 4\times\unit{9,81}{\meter/\second^2}$ peak-to-peak) eingestellt,
der Gyrosensor auf $\pm$ \unit{2000}{\degree/\second}. Letzterem liegt die Annahme zugrunde, dass eine schnelle Handbewegung eine Rotation um \unit{180}{\degree} 
innerhalb von \unit{100}{\milli\second} bewirken kann, was einer Winkelgeschwindigkeit von \unit{1800}{\degree/\second} entspräche (siehe dazu auch Abschnitt~\ref{subsec:mpu6050_lessonslearned} auf
Seite~\pageref{lessonslearned:drehrate}). 
Beide Sensoren wurden für eine Samplerate von \unit{200}{\hertz} konfiguriert, was einen Kompromiss zwischen Datenaktualität und Verarbeitungsgeschwindigkeit bildet. Während des Debuggings
wurde diese Geschwindigkeit auf \unit{100}{\hertz} begrenzt.
Die Triggerung der Verbarbeitung wurde auch hier über eine Interruptleitung vorgenommen, welche auf open drain, active low geschaltet wurde, wobei das Zurücksetzen des Interrupts beim Auslesen
des Statusregisters geschieht.
...
\paragraph{Fehler bei Kaltstart} Bei der Kommunikation mit der MPU6050 trat das Problem auf, dass bei aktiviertem Interrupt-Signal und reduzierter Samplingrate nach einem Kaltstart keinerlei nennenswerte 
...
\paragraph{Sättigung der Drehraten} Eine\label{lessonslearned:drehrate} initiale (zu geringe) Einschätzung über die möglichen Rotationsgeschwindigkeiten führte zu einem Betrieb des Gyrosensors im Sättigungsbereich, was die Fusion deutlich beeinträchtigte;
Da den Gyrosensordaten in der Fusion i.Allg. mehr Vertrauen geschenkt wird als den (oft ungenaueren) Accelerometerdaten (vgl. Abschnitt~\ref{subsec:calibrationdata}) wird die fusionierte Rotation vorzeitig stark verlangsamt,
...
\begin{table}[htbp]
	\centering
		\begin{tabular}{ll}
		\toprule
		\multicolumn{2}{l}{Eigenschaften des Honeywell HMC5883L} \\
		\midrule
		Messbereich &  $\pm$ \unit{8}{\gauss}\footnote{Gauss} \\
		Samplingrate &  \unit{160}{\hertz} \\
		Auflösung & 12-Bit\\
		Grundrauschen & \unit{2}{\milli\gauss} \\
		Versorgungsspannung (VDD) & \unit{2,16}{\volt} bis \unit{3,6}{\volt} \\
		Schnittstellenspannung & \unit{1,71}{\volt} bis \unit{VDD}{\volt} \\
		Stromverbrauch & \unit{100}{\micro\ampere} (Idle \unit{2}{\micro\ampere})\\
		Betriebstemperatur & \unit{-30}{\celsius} bis \unit{+85}{\celsius} \\
		\bottomrule
		\end{tabular}
	\caption{Eigenschaften des Honeywell HMC5883L}
	\label{tab:HMC5883LEigenschaften}
\end{table}
...
Eine Unschönheit bei der Verwendung des HMC5883L ist die Anordnung der \gls{i2c}-Register; So existiert zwar ein Register, welches das Vorliegen neuer Sensordaten signalisiert, dieses ist jedoch nicht
effizient verwendbar. Tabelle~\ref{tab:HMC5883LRgister} zeigt die Auflistung der Register und ihre Adressen.

\begin{table}[htbp]
	\centering
		\begin{tabular}{ll}
		\toprule
		\multicolumn{2}{l}{\gls{i2c}-Register des Honeywell HMC5883L} \\
		\midrule
		...
		$05$ & Data Output Y MSB Register \\
		$06$ & Data Output Y LSB Register \\
		$07$ & Data Output Z MSB Register \\
		$08$ & Data Output Z LSB Register \\
		\midrule
		$09$ & Status Register \\
		...
		\bottomrule
		\end{tabular}
	\caption{\gls{i2c}-Register des Honeywell HMC5883L}
	\label{tab:HMC5883LRgister}
\end{table}

Das Handbuch des HMC5883L (\citealp{hmc5883l}) formuliert dazu:

\begin{quote}
To minimize the communication between the master and this device, the address pointer updated automatically without master intervention. This automatic address pointer update has two additional features. 
First when address 12 or higher is accessed the pointer updates to address 00 and secondly when address 08 is reached, the pointer rolls back to address 03.
\end{quote}

Dass diese "`feature"' zur Minimierung der Kommunikation einen gegenteiligen Effekt haben, wird deutlich, wenn man die tatsächlich notwendige Kommunikation auf dem \gls{i2c}-Bus betrachtet:

Ein kontinuierlicher Lesevorgang ab Register \emph{03} führt dazu, dass das Statusregister nicht ausgewertet werden kann, da ab Register \emph{08} ein Sprung zurück auf Register \emph{03} erfolgt.
Wird der Lesevorgang an Register \emph{09} (dem Statusregister) gestartet, dauert es jedoch 8 Lesezyklen, bis das erste Datenregister erreicht ist (durch den Sprung ab Register \emph{12}).

Der implementierte Mechanismus ist daher, einen Lesezugriff auf Register \emph{09} zu initiieren, was den Kommandos \texttt{0x3C, 0x0C, 0x3D, 0x00}\footnote{\texttt{0x3C} enspricht einem Write-Befehl mit der Gerätekennung \texttt{0x1E} des Sensors auf dem \gls{i2c}-Bus, \texttt{0x3D} entsprechend dem Read.} entspricht, wobei das letzte Byte lediglich die
\gls{i2c}-Clock treibt. Hiernach wird (abhängig vom Registerinhalt) ein \emph{restart} ausgelöst, ohne den Bus freizugeben, und der Lesezeiger auf Register \emph{03} gesetzt, was den Kommandos
\texttt{0x3C, 0x03, 0x3D, 0x00,...} entspricht.

Über die zugrundeliegenden Entscheidungen zum Design des Registerlayouts seitens des Herstellers lässt sich nur spekulieren, jedoch erscheint eine Anordnung des Statusregisters vor den 
Datenregistern (d.h. zwischen Register \emph{02} und \emph{03}) deutlich sinnvoller. Das Verwerfen der Information des Statusregisters ist ob der infrequenten Aktualisierung der Messwerte
gefährlich, da die maximale Samplerate nur \unit{75}{\hertz} bei Verwendung der "`schnellsten"' Einstellung\footnote{\texttt{DO2..0 = 0b110} im Configuration Register A.} beträgt. 
Für die angegebene, maximale Samplerate von \unit{160}{\hertz} soll der externe Interrupt beobachtet werden (welcher jedoch auf dem Breakout Board nicht zur Verfügung steht.) 
Inwiefern dies mit der Modusbeschreibung in Einklang zu bringen ist, geht aus dem Handbuch nicht hervor.
...
\paragraph{Reihenfolge der Messwerte}
...
\paragraph{Interpretation der unkalibrierten Messergebnisse}
...
\subsection{Kalibrierungsdaten}
\label{subsec:calibrationdata}

Die Ausgabe der in MATLAB gewonnenen Kalibrierungsdaten (vgl. Abbildung~\ref{fig:sensors_unagitated}) sind in Listing~\ref{lst:sensor-calibration-data} zu finden.
Zu erkennen sind die aus dem Ellipsoid-Fitting ermittelten affinen Transformationsmatrizen, sowie die Varianzen der Messwerte. 

Die vergleichsweise hohe Varianz der Messdaten des Gyrosensors ist hierbei (wortwörtlich) differenziert zu betrachten, da sie die Varianz der Winkel\emph{geschwindigkeiten}, d.h. der Differentiale der Winkel beschreibt.
Eine Analyse der Definitionen der Integrale der Kosinus- und Sinusfunktionen (Gleichungen~\ref{eq:intcos} und~\ref{eq:intsine}) zeigt, dass jede überlagerte Schwingung einer Frequenz $f > \unit{1}{\hertz}$
bei der Integration um den Faktor $2 \pi f$ gedämpft wird. 

\begin{align}
\int cos(2 \pi f t) &= \quad \frac{1}{2 \pi f} sin(2 \pi f t) \label{eq:intcos} \\
\int sin(2 \pi f t) &= -\frac{1}{2 \pi f} cos(2 \pi f t) \label{eq:intsine}
\end{align}

Dies ist auch die Begründung für die starke Drift der Messwerte eines Gyrosensors: Analog zur obigen Betrachtung bewirken Schwingungen (d.h. Bewegungen mit) einer Frequenz $f < \unit{1}{\hertz}$ 
bei der Integration eine Verstärkung derselben Schwingung, was sich als langsame Offsetveränderung der Messwerte äußert.

Ein weiterer Aspekt ist, dass Accelerometer trotz ihrer guten Performance in Ruhelage entsprechend ihrer Natur sehr sensibel gegenüber Bewegungen sind, was die Hauptursache für den "`Messfehler"'
ausmacht. Gyrosensoren sind aufgrund ihrer nichtlinearen Natur von diesem Problem (bei linearen Bewegungen) nicht betroffen; Nicht zuletzt, sie auch mit deutlich höheren Sampleraten betrieben 
werden können als Accelerometer, wobei dieser Vorteil jedoch im Projekt nicht ausgenutzt wurde.

\begin{lstlisting}[style=lolmatlab,caption={Ausgabe des Kalibrierungsskriptes},label=lst:sensor-calibration-data]
Loading magnetometer calibration data.
 
Affine hard- and soft-iron correction matrix:
  0.98308   0.0025144     0.02777   0.0064502
0.0025144     0.92661   -0.043022     0.10543
  0.02777   -0.043022      1.1128   -0.020258
        0           0           0           1
Calculated axis variances: 
2.0347e-06  1.9233e-06  2.3021e-06

Loading accelerometer calibration data.
 
Affine correction matrix from ellipsoid fitting:
    1.0062   0.0034341  -0.0027532     -0.0164
 0.0034341      1.0008  -0.0056162   -0.016173
-0.0027532  -0.0056162      0.9931     0.02059
         0           0           0           1
Calculated axis variances: 
9.8036e-06  9.6462e-06  2.4831e-05

Loading gyroscope calibration data.
...
0           0           0           1
Calculated axis variances: 
0.016307   0.0084706      0.0129
Calculated axis offsets: 
4.5446    0.048858      1.1197
\end{lstlisting}
